  ...
  return (
    ...
    >
      <div className="bg-white p-8 rounded shadow-md max-w-sm w-full relative">
        ...
        <h2 className="text-2xl font-bold mb-6 text-center">{t('apply_job')}</h2>
        <div className="space-y-1">
          <p><strong>{t('company')}:</strong> {job.companyName}</p>
          <p><strong>{t('job_name')}:</strong> {job.name}</p>
          <p><strong>{t('created_at')}:</strong> {new Date(job.createdAt).toLocaleDateString()}</p>
          <p><strong>{t('location')}:</strong> {job.location}</p>
          <div className='flex'>
            <span><strong>{t('keywords')}:</strong></span>
          </div>
          ...
          <h1><strong>{t('job_description')}</strong></h1>
